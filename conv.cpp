...
static inline void fast_set_val( float * a, long d, const float val) {
  if(val) {
    int j;
    for(j=0; j<d; j++)
      a[j] = val;
...
static inline void fast_add_val( float * a, long d, const float val) {
  int j;
  for(j=0; j<d; j++)
    a[j] += val;
}
static inline void fast_set_vec( float * dest,
                                 const float * src, int d, const float mul) {
  if( mul==1) 
...
static inline void fast_add_vec( float * dest,
                                 const float * add, int d, float mul) {
  if(d<=4) {
    int j;
    for(j=0; j<d; j++)
      dest[j] += mul*add[j];
...
static inline void fast_div( float * a, long d, const float div) {
