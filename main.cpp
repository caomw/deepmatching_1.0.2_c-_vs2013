...
image_t* image_gray_from_mat(const cv::Mat &m)
{
	image_t* res = image_new(m.cols, m.rows);

	int n = 0;
	for (int j = 0; j < m.rows; j++)
	{
		for (int i = 0; i < m.cols; i++, n++)
		{
			res->data[i + j*res->stride] = m.data[n];
		}
	}
	return res;
}
...
{
	cv::Mat cim1 = cv::imread(name, 0);
	image_t *im1 = image_gray_from_mat(cim1);
